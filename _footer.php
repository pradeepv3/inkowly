<footer class="sec-footer-wrapper">
	<div class="sec-footer clearfix ">
		<div class="container">
			<div class="col-sm-6 col-md-3 navbar navbar-default sec-footer__intouch clearfix">
				<h4>Site Navigation</h4>
				<ul id="" class="nav navbar-nav">
					<li><a href="index.php"><span class="fa fa-caret-right"></span> Home</a></li>
					<li><a href="sub-page.php?page=about-us"><span class="fa fa-caret-right"></span> About Us</a></li>
					<li><a href="sub-page.php?page=services"><span class="fa fa-caret-right"></span>  Services</a></li>
					<li><a href="sub-page.php?page=our-team"><span class="fa fa-caret-right"></span>  Our Team</a></li>
					<li><a href="sub-page.php?page=contact-us"><span class="fa fa-caret-right"></span>  Contact</a></li>
				</ul>
			</div>
			
			<div class="col-md-3 hidden-sm hidden-xs  clearfix">
				
			</div>
			<div class="col-sm-6 col-md-3 hidden-xs navbar clearfix">
				<h4>Get in touch</h4>
				<p>Add us in popular social network to get more update.</p>
				<ul class="nav navbar-nav social">
					<li><span class="fa fa-facebook"></span></li>
					<li><span class="fa fa-twitter"></span></li>
					<li><span class="fa fa-google"></span></li>
					<li><span class="fa fa-linkedin"></span></li>
					<li><span class="fa fa-rss"></span></li>
				</ul>
			</div>
			<div class="col-md-3 hidden-sm hidden-xs clearfix">
				<h4>Our Contacts</h4>
				<address class="address"> <span class="fa fa-map-marker"></span>
					Cheldol<br>
					P Bag BR63 Broadhurt, <br>
					Gaborone, Botswana.<br>
				</address>
				<p><span class="fa fa-phone"></span>+297 0000 0000</p>
				<p><span class="fa fa-envelope"></span>+297 0000 0000</p>
				<p><span class="fa fa-fax"></span><a href="mailto:chemdol@info.bw">chemdol@info.bw</a></p>
			</div>
		</div>
	</div>

	<div class="sec-copyright">
		<div class="container">
			Copyright &copy; <?php echo date('Y') ?> | All right Reserved
		</div>
	</div>
</footer>
</div>
	<script type='text/javascript' src='js/bootstrap.min.js'></script>
	<script type="text/javascript" src="js/wow.js"></script>
	<script type="text/javascript" src="//cdnjs.cloudflare.com/ajax/libs/mixitup/2.1.11/jquery.mixitup.js"></script>
	<script type="text/javascript" src="js/jquery.fancybox.js"></script>
	<script type='text/javascript' src='js/script.js'></script>
</body>
</html>