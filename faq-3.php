<?php include("modules/common/_header.php"); ?>

	<section class="section sec-subpage-banner">
		
		<div class="container">
			
			<div class="col-sm-8">
				<h2 class="page-title">FAQ</h2>
			</div>
			<div class="col-sm-4">
				<ul class="breadcrumb">
					<li><a href="">Home</a></li>
					<li class="active">FAQ</li>
				</ul>
			</div>
		</div>
	</section>
	<section class="m-content-wrapper sec-subpage-main sec-faq">
		
		<div class="section br-sec-line text-center">
			<div class="container">
				<div class="col-sm-10 col-sm-offset-1">
					<p class="txt-lg">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Similique excepturi voluptates placeat ducimus delectus magnam tempore dolore dolorem quisquam porro modi voluptatum eum saepe dolorum.</p>
				</div>
			</div>
		</div>

		<div class="section clearfix">
			<div class="container">
				<div class="col-sm-12 faq-container">
					<div class="faq-container__questions" id="faq_questions">
						<ul class="list-unstyled">
							<li class="question"><a href="#q1" >1. Lorem ipsum dolor sit amet, consectetur adipiscing elit?</a></li>
							<li class="question"><a href="#q2" >2. Donec sed diam at enim pellentesque aliquam?</a></li>
							<li class="question"><a href="#q3" >3. Sed sit amet lectus velit. Etiam at massa ac turpis tristique eleifend?</a></li>
							<li class="question"><a href="#q4" >4. Etiam libero est, aliquam at tincidunt at, hendrerit non nulla?</a></li>
							<li class="question"><a href="#q5" >5. Nunc lorem velit, ultricies vitae lectus eu, laoreet congue augue?</a></li>
							<li class="question"><a href="#q6" >6. Ut sed tempor ex, et fringilla nunc. Fusce varius lectus erat, in euismod libero mollis ut?</a></li>
							<li class="question"><a href="#q7" >7. Donec sed euismod magna, ac sodales diam. Class aptent taciti sociosqu ad litora torquent per conubia nostra?</a></li>
							<li class="question"><a href="#q8" >8. Morbi libero sem, sagittis sit amet euismod vitae, mattis eget ex?</a></li>
							<li class="question"><a href="#q9" >9. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos?</a></li>
							<li class="question"><a href="#q10" >10. Suspendisse maximus eget justo ac egestas. In hac habitasse platea dictumst?</a></li>
						</ul>
					</div>
					<div class="faq-container__answers">
						<div id="q1" class="faq-container__answer">
							<h3 class="question">1. Lorem ipsum dolor sit amet, consectetur adipiscing elit?</h3>
							<div class="answer">
								<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Doloribus nesciunt ut officiis accusantium quisquam minima praesentium, beatae fugit illo nobis fugiat adipisci quia distinctio repellat culpa saepe, optio aperiam est!</p>
								<p><strong>Example: </strong>Facere, id excepturi iusto aliquid beatae delectus nemo enim, ad saepe nam et.</p>
							</div>
						</div>
						<div id="q2" class="faq-container__answer">
							<h3 class="question">2. Donec sed diam at enim pellentesque aliquam?</h3>
							<div class="answer">
								<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Doloribus nesciunt ut officiis accusantium quisquam minima praesentium, beatae fugit illo nobis fugiat adipisci quia distinctio repellat culpa saepe, optio aperiam est!</p>
								<p><strong>Example: </strong>Facere, id excepturi iusto aliquid beatae delectus nemo enim, ad saepe nam et.</p>
							</div>
						</div>
						<div id="q3" class="faq-container__answer">
							<h3 class="question">3. Sed sit amet lectus velit. Etiam at massa ac turpis tristique eleifend?</h3>
							<div class="answer">
								<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Doloribus nesciunt ut officiis accusantium quisquam minima praesentium, beatae fugit illo nobis fugiat adipisci quia distinctio repellat culpa saepe, optio aperiam est!</p>
								<p><strong>Example: </strong>Facere, id excepturi iusto aliquid beatae delectus nemo enim, ad saepe nam et.</p>
							</div>
						</div>
						<div id="q4" class="faq-container__answer">
							<h3 class="question">4. Etiam libero est, aliquam at tincidunt at, hendrerit non nulla?</h3>
							<div class="answer">
								<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Doloribus nesciunt ut officiis accusantium quisquam minima praesentium, beatae fugit illo nobis fugiat adipisci quia distinctio repellat culpa saepe, optio aperiam est!</p>
								<p><strong>Example: </strong>Facere, id excepturi iusto aliquid beatae delectus nemo enim, ad saepe nam et.</p>
							</div>
						</div>
						<div id="q5" class="faq-container__answer">
							<h3 class="question">5. Nunc lorem velit, ultricies vitae lectus eu, laoreet congue augue?</h3>
							<div class="answer">
								<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Doloribus nesciunt ut officiis accusantium quisquam minima praesentium, beatae fugit illo nobis fugiat adipisci quia distinctio repellat culpa saepe, optio aperiam est!</p>
								<p><strong>Example: </strong>Facere, id excepturi iusto aliquid beatae delectus nemo enim, ad saepe nam et.</p>
							</div>
						</div>
						<div id="q6" class="faq-container__answer">
							<h3 class="question">6. Ut sed tempor ex, et fringilla nunc. Fusce varius lectus erat, in euismod libero mollis ut?</h3>
							<div class="answer">
								<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Doloribus nesciunt ut officiis accusantium quisquam minima praesentium, beatae fugit illo nobis fugiat adipisci quia distinctio repellat culpa saepe, optio aperiam est!</p>
								<p><strong>Example: </strong>Facere, id excepturi iusto aliquid beatae delectus nemo enim, ad saepe nam et.</p>
							</div>
						</div>
						<div id="q7" class="faq-container__answer">
							<h3 class="question">7. Donec sed euismod magna, ac sodales diam. Class aptent taciti sociosqu ad litora torquent per conubia nostra?</h3>
							<div class="answer">
								<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Doloribus nesciunt ut officiis accusantium quisquam minima praesentium, beatae fugit illo nobis fugiat adipisci quia distinctio repellat culpa saepe, optio aperiam est!</p>
								<p><strong>Example: </strong>Facere, id excepturi iusto aliquid beatae delectus nemo enim, ad saepe nam et.</p>
							</div>
						</div>
						<div id="q8" class="faq-container__answer">
							<h3 class="question">8. Morbi libero sem, sagittis sit amet euismod vitae, mattis eget ex?</h3>
							<div class="answer">
								<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Doloribus nesciunt ut officiis accusantium quisquam minima praesentium, beatae fugit illo nobis fugiat adipisci quia distinctio repellat culpa saepe, optio aperiam est!</p>
								<p><strong>Example: </strong>Facere, id excepturi iusto aliquid beatae delectus nemo enim, ad saepe nam et.</p>
							</div>
						</div>
						<div id="q9" class="faq-container__answer">
							<h3 class="question">9. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos?</h3>
							<div class="answer">
								<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Doloribus nesciunt ut officiis accusantium quisquam minima praesentium, beatae fugit illo nobis fugiat adipisci quia distinctio repellat culpa saepe, optio aperiam est!</p>
								<p><strong>Example: </strong>Facere, id excepturi iusto aliquid beatae delectus nemo enim, ad saepe nam et.</p>
							</div>
						</div>
						<div id="q10" class="faq-container__answer">
							<h3 class="question">10. Suspendisse maximus eget justo ac egestas. In hac habitasse platea dictumst?</h3>
							<div class="answer">
								<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Doloribus nesciunt ut officiis accusantium quisquam minima praesentium, beatae fugit illo nobis fugiat adipisci quia distinctio repellat culpa saepe, optio aperiam est!</p>
								<p><strong>Example: </strong>Facere, id excepturi iusto aliquid beatae delectus nemo enim, ad saepe nam et.</p>
							</div>
						</div>
					</div>				
				</div>
				</div>
			</div>

	</section>
	<div id="move-top" class="move-top"><span class="fa fa-angle-double-up"></span></div>
<?php include("modules/common/_footer.php"); ?>